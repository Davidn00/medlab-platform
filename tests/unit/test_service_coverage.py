...
def test_notification_service_creates_only_new_expired_notifications():
    ...
    created = service.notify_calibration_expired(
        calibration
    )
    ...
    assert (
        repository.create_if_not_exists.call_count
        == 2
    )
...
def test_notification_service_creates_expiring_notifications():
    ...
    repository.create_if_not_exists.return_value = (
        MagicMock()
    )
    ...
    assert (
        ...
        == NotificationType.CALIBRATION_EXPIRING
    )
    ...
    assert (
        "expiring:2026-02-01"
        in notification.deduplication_key
    )
...
def test_notification_service_delegates_queries():
    ...
    repository.get_for_user.return_value = [
        "n1"
    ]
...
def test_calibration_service_create_validates_equipment_and_dates():
    ...
    equipment_repo.get_by_id.return_value = (
        SimpleNamespace(id="eq-1")
    )
...
def test_calibration_service_create_rejects_missing_equipment_and_bad_dates():
    ...
    equipment_repo.get_by_id.assert_called_once_with(
            "missing"
    )
...
def test_calibration_service_queries_and_expiration_window():
    ...
    repo.get_by_equipment_id.return_value = [
        calibration
    ]

    repo.get_expiring_calibrations.return_value = [
        calibration
    ]

    repo.get_expired_calibrations.return_value = [
        calibration
    ]

    assert (
        service.get_by_id("c1")
        is calibration
    )
    ...
    assert (
        service.get_by_equipment_id("eq-1")
        == [calibration]
    )

    assert (
        service.get_expiring_calibrations(30)
        == [calibration]
    )

    assert (
        service.get_expired_calibrations()
        == [calibration]
    )
